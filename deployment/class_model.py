...
class ModelClassDep:
    ...
    def model_execution(self):
        ...
        pipe = StableDiffusionInpaintPipeline.from_pretrained(model_dir, scheduler=scheduler, variant="fp16",torch_dtype=torch.float16)
        pipe = pipe.to("cuda")
        ...
        # source_image = source_image.resize((max_dimension, max_dimension), Image.LANCZOS)
        ...
        # If either dimension exceeds 600 pixels, reduce the quality while keeping the aspect ratio
        max_dimension = max(width, height)
        if max_dimension > 800:
            source_image.thumbnail((800, 800), Image.LANCZOS)
            width, height = source_image.size
            new_size = (800, 800)
            source_image = source_image.resize(new_size, Image.LANCZOS)
        ...
        # pil_image = create_image_grid(source_image, encoded_images, inpainting_prompts, 2, 3)
        pil_image = encoded_images[0]
        result_image_path = 'static/result_image.png'  # Change this to your desired file path
